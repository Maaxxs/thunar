...
/* TODO Questions:
 * - inherit from ThunarAbstractIconView or from ThunarStandardView like
 *   ThunarDetailsView ? 
 * - 
 *
 */
...
static void         thunar_miller_view_set_property         (GObject             *object,
                                                             guint                prop_id,
                                                             const GValue        *value,
                                                             GParamSpec          *pspec);
static AtkObject   *thunar_miller_view_get_accessible       (GtkWidget           *widget);
static void         thunar_miller_view_zoom_level_changed   (ThunarStandardView  *standard_view);


struct _ThunarMillerViewClass
{
  ThunarAbstractIconViewClass __parent__;
};

struct _ThunarMillerView
{
...
/* G_DEFINE_TYPE (newtype, new_type, G_TYPE_OBJECT parent) */
/* defines the _get_type() function! */
G_DEFINE_TYPE (ThunarMillerView, thunar_miller_view, THUNAR_TYPE_ABSTRACT_ICON_VIEW)



static void
thunar_miller_view_class_init (ThunarMillerViewClass *klass)
{
  ...
  gobject_class->set_property = thunar_miller_view_set_property;

  gtkwidget_class = GTK_WIDGET_CLASS (klass);
  gtkwidget_class->get_accessible = thunar_miller_view_get_accessible;
...
static void
thunar_miller_view_init (ThunarMillerView *miller_view)
{
  /* setup the icon renderer */
  g_object_set (G_OBJECT (THUNAR_STANDARD_VIEW (miller_view)->icon_renderer),
                "ypad", 3u,
                NULL);

  /* setup the name renderer */
  g_object_set (G_OBJECT (THUNAR_STANDARD_VIEW (miller_view)->name_renderer),
                "wrap-mode", PANGO_WRAP_WORD_CHAR,
                NULL);

  /* synchronize the "text-beside-icons" property with the global preference */
  exo_binding_new (G_OBJECT (THUNAR_STANDARD_VIEW (miller_view)->preferences), "misc-text-beside-icons", G_OBJECT (miller_view), "text-beside-icons");
}



static void
thunar_miller_view_set_property (GObject      *object,
                               guint         prop_id,
                               const GValue *value,
                               GParamSpec   *pspec)
{
      ...
      if (G_UNLIKELY (g_value_get_boolean (value)))
        {
          ...
          g_signal_handlers_disconnect_by_func (object, thunar_miller_view_zoom_level_changed, NULL);
        }
      else
        {
          ...
          g_signal_connect (object, "notify::zoom-level", G_CALLBACK (thunar_miller_view_zoom_level_changed), NULL);
          thunar_miller_view_zoom_level_changed (standard_view);
        }
      break;
...
static AtkObject*
thunar_miller_view_get_accessible (GtkWidget *widget)
{
  AtkObject *object;

  /* query the atk object for the miller view class */
  /* or is 'thunar_icon_view_parent_class' ? */
  object = (*GTK_WIDGET_CLASS (thunar_miller_view_parent_class)->get_accessible) (widget);
...
static void
thunar_miller_view_zoom_level_changed (ThunarStandardView *standard_view)
{
  gint wrap_width;
